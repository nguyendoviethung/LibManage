...
export default function ReaderManagement() {
  ...
  const handleAddReader = async (data) => {
    try {
      ...
      setAlertBox({ message: 'Lỗi thêm người dùng', type: 'error' });
      return false;
    }
  };
          ...
          <Table striped bordered hover responsive className="custom-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} style={{textAlign:'center', padding:'48px 0', color:'#888'}}>Không tìm thấy người dùng nào phù hợp.</td></tr>
              ) : (
                readers.map((r, index) => (
                  <tr key={r.student_id}>
        ...
        <Modal.Body>
          {detailReader && (
            <div style={{ lineHeight: '1.8' }}>
              <div><b>MSSV:</b> {detailReader.student_id}</div>
              <div><b>Họ tên:</b> {detailReader.full_name}</div>
              <div><b>Email:</b> {detailReader.email}</div>
              <div><b>Số điện thoại:</b> {detailReader.phone_number}</div>
              <div><b>Khoa:</b> {detailReader.faculty}</div>
              <div><b>Trạng thái:</b> {detailReader.status}</div>
            </div>
          )}
        </Modal.Body>
      </Modal>
