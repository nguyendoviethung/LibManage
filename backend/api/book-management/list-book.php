...
header('Access-Control-Allow-Methods: POST,GET');
  ...
  $sortKey = $data['sortKey'] ?? 'book_id';
  $allowedSortKeys = ['book_id', 'title', 'author_name', 'publisher_year', 'quantity', 'created_at'];
  if (!in_array($sortKey, $allowedSortKeys)) $sortKey = 'book_id';
  ...
  if (!$result_1) {
    echo json_encode(["success" => false, "message" => "Lỗi truy vấn danh sách"]);
    exit;
  }
